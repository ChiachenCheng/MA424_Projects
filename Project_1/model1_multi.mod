...
var unfulfill{workstation, SKU} >=0, integer;
...
+ alpha2 * (sum {w in workstation,r in rack} distwr[w,r]*x[w,r]) + alpha3 * (sum {w in workstation, s in SKU} unfulfill[w,s]);

subject to
unfulfilled{w in workstation, s in SKU}: unfulfill[w,s] >= demand[w,s]-supply[w,s];
